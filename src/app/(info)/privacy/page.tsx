...
export const metadata: Metadata = {
  ...pageMetadata({
    title: "プライバシーポリシー",
    description:
      "中国料理 愚問（貝塚）のプライバシーポリシー。アクセス解析（Googleアナリティクス）・埋め込み地図・ネット予約カレンダー（ホットペッパーグルメ）による外部送信の内容、Cookieの取り扱い、ご予約でお預かりする情報の扱いについて。",
    path: "/privacy",
  }),
...
const TRANSMISSIONS = [
  ...
  {
    name: "ホットペッパーグルメ（ネット予約 空席カレンダー）",
    operator: "株式会社リクルート",
    sent: "IPアドレス、端末・ブラウザの種類、ホットペッパーグルメのサービスで保存された Cookie。予約手続きで入力されたお名前・連絡先・人数・日時は、当サイトを経由せずホットペッパーグルメに直接送られます",
    purpose: "空席状況の表示と、ネット予約の受付のため（カレンダーが画面に表示されたときに読み込まれます）",
    where: "お問い合わせのページ",
  },
] as const;
      ...
      <div className="gm-info-body">
        <InfoSection eyebrow="POLICY" title="このサイトについて">
          <div className="gm-info-prose" data-info-row>
            ...
            <p>
              当サイト自身が、お名前やご連絡先を入力していただく仕組みを持つことはありません。
              ご予約・お問い合わせはお電話（{TEL_DISPLAY}）で承っています。
              お問い合わせのページのネット予約カレンダーでご入力いただく内容は、
              ホットペッパーグルメ（株式会社リクルート）が直接お預かりします。
            </p>
          </div>
        </InfoSection>
